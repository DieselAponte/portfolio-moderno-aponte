...
interface AddInfoExperienceProps {
    data: { titulo: string; descripcion: string; company: string };
    onChange: (data: { titulo: string; descripcion: string; company: string }) => void;
    onContinue: () => void;
    error: string;
}

export const AddInfoExperience = ({ data, onChange, onContinue, error }: AddInfoExperienceProps) => {
    return (
        <div className="space-y-5">
            <div>
                ...
                <input type="text" value={data.titulo} onChange={(e) => onChange({ ...data, titulo: e.target.value })}
                    placeholder="Ej: Front-end Engineer"
            ...
            <div>
                ...
                <textarea value={data.descripcion} onChange={(e) => onChange({ ...data, descripcion: e.target.value })}
                    placeholder="Describe tu rol y experiencia..." rows={4}
            ...
            <div>
                ...
                <input type="text" value={data.company} onChange={(e) => onChange({ ...data, company: e.target.value })}
                    placeholder="Ej: Aperture Science"
                    className="w-full rounded-lg border border-aperture-gray bg-aperture-dark/80 px-4 py-2.5 text-sm text-white placeholder-zinc-600 outline-none transition focus:border-aperture-blue/60" />
            </div>
